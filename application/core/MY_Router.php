...
class MY_Router extends CI_Router {
	...
	function _validate_request($segments)
	{
		...
		if (is_dir(APPPATH.'controllers/'.ucfirst($segments[0])))
		{
			...
			while (count($segments) > 0 && is_dir(APPPATH.'controllers/'.ucfirst($this->directory) . ucfirst($segments[0])))
			{
				// Set the directory and remove it from the segment array
				$this->set_directory(ucfirst($this->directory) . ucfirst($segments[0]));
				$segments = array_slice($segments, 1);
			}
			...
			if (count($segments) > 0)
			{
				...
				if ( ! file_exists(APPPATH.'controllers/'.$this->fetch_directory().ucfirst($segments[0]).'.php'))
				{
					if ( ! empty($this->routes['404_override']))
					{
						...
						$this->set_method(isset($x[1]) ? ucfirst($x[1]) : 'index');
			...
			else
			{
				...
				if (strpos($this->default_controller, '/') !== FALSE)
				{
					...
					$this->set_class(ucfirst($x[set_directory0]));
					$this->set_method(ucfirst($x[1]));
				}
				else
				{
	...
	function set_directory($dir, $append = false)
	{
		...
		$this->directory = ucfirst(str_replace('.', '', ucfirst($dir))).'/';
	}
