...
{    
  ...
  function cetak($tahun, $periode, $tglStart,$tglEnd, $cetak){
       ...
       $this->db->where('t_proposal_tahun', $tahun);
       $this->db->where('t_proposal_periode', $periode);
       $this->db->where('t_tok_tgl >=', $tglStart);
       $this->db->where('t_tok_tgl <=', $tglEnd);
       $this->db->join(self::$table, 't_tok_proposal=t_proposal_id', 'left');
       ...
       $this->db->join(self::$table6, 't_tok_tempat=m_tempat_id', 'left');
       $this->db->order_by('t_tok_tgl', 'ASC')
				->order_by('t_tok_waktu_dari', 'ASC')
