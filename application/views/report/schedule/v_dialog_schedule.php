...
<!-- Form -->
    <form id="fm-dialog_schedule" method="post" novalidate buttons="#dlg_btn-dialog_schedule">
        <div class="fitem">
        ...
        <div class="fitem">
            <label for="type">Schedule Dari Tgl </label>
            <input type="text" id="tglStart" name="tglStart" class="easyui-datebox" required="true"/>
        </div>
        <div class="fitem">
            <label for="type">Schedule Sampai Tgl </label>
...
<!-- Dialog Button -->
<div id="dlg_btn-dialog_schedule">
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" iconCls="icon-ok" onclick="cetak_schedule();">Cetak</a>
...
<script type="text/javascript">
    ...
    function cetak_schedule(){
        var isValid = $('#fm-dialog_schedule').form('validate');
        if (isValid){
            ...
            var url = '<?php echo site_url('report/schedule/cetak'); ?>/'+tahun+'_'+periode+'_'+tglStart+'_'+tglEnd+'_'+cetak;
            var content = '<iframe scrolling="auto" frameborder="0"  src="'+url+'" style="width:100%;height:100%;"></iframe>';
            var title   = 'Cetak Schedule '+tahun;
            if ($('#tt').tabs('exists', title))
            {
                $('#tt').tabs('select', title);
                $('#fm-dialog_schedule').dialog('close');
            } 
            else 
            {
                $('#tt').tabs('close', title);
...
<!-- End of file v_dialog_schedule.php -->
<!-- Location: ./views/report/schedule/v_dialog_schedule.php -->
